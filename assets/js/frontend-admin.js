(function($){
	...
	var FrontendValidator = acf.Model.extend(
		{
			...
			showErrors: function () {
				...
				var customErrorMessage = this.$el.data( 'error-message' );
	...
	$( 'body' ).on(
		...
		'.fea-scroll-right',
		function(e){
			feaScroll( $( this ), 'right' );
		}
	);


/* 	$('body').on('click', 'lightbox-image', function(e) {
		e.preventDefault();
		var $el = $(this);
		var url = $el.attr('data-url');
		var $lightbox = $('<div class="fea-lightbox"><div class="fea-lightbox-content"><span class="fea-close acf-icon -cancel"></span><img src="' + url + '"></div></div>');
		$('body').append($lightbox);

		//images take from this image and all it's siblings


		$lightbox.find('.fea-close').on('click', function() {
			$lightbox.remove();
		});
		$lightbox.show();
	});
	 */
	function feaScroll( $el, $direction ){
		var $scroll = $el.siblings( 'div' );
		var $scrollLeft = $scroll.scrollLeft();
		var $scrollSpeed = 10;

		if( typeof feaAttachmentInterval != 'undefined' ) clearInterval( feaAttachmentInterval );

		
		feaAttachmentInterval = setInterval(
			function(){
				if ( $direction == 'left' ) {
					$scroll.scrollLeft( $scrollLeft += $scrollSpeed );
				} else {
					$scroll.scrollLeft( $scrollLeft -= $scrollSpeed );
				}
				$( 'body' ).on(
					'mouseup touchend',
					function(){
						clearInterval( feaAttachmentInterval );
					}
				);
			},
			10
	...
	acf.frontendFormSuccess = function(response){
		if (response.success) {
			...
			$form.prepend( '<div class="frontend-admin-message"><div class="acf-notice -error acf-error-message"><p class="error-msg">' + response.data + '</p><span class="frontend-admin-dismiss close-msg acf-notice-dismiss acf-icon -cancel small"></span></div></div>' );	
		}
		acf.unlockForm( $form );
			...
			filePreviews: function( e, $el ){
				...
				Object.keys( files ).forEach(
					i => {
						...
						const file    = files[i];
						// check feadata.server_upload_size and see if it allows the file upload						
						var container = this.$clone().clone();
						let maxSize   = control.data( 'max_size' );

						var fileSize = $.bytesToMegaBytes( file.size );
						if ((maxSize && fileSize > maxSize ) 
							|| fileSize > feadata.server_upload_size 
							|| fileSize > feadata.server_post_size) 
						{						
							self.showNotice(
								{
