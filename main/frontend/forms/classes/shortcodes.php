...
if ( ! class_exists( 'Dynamic_Values' ) ) :
	...
	class Dynamic_Values {
		...
		function render_field_display( $field, $source = 'current' ){
			...
			global $fea_scripts, $fea_fields, $post;

			if( $post && 'current' == $source ){
				$source = $post->ID;
			}
			...
			switch ( $field['type'] ) {
				...
				case 'site_logo':
					...
					case 'product_images':
						...
						<span class="fea-scroll-right dashicons dashicons-arrow-left-alt2"></span>';
						...
						$return .= '</div>';
						$return .= '<span class="fea-scroll-left dashicons dashicons-arrow-right-alt2"></span></div>';
					break;
				case 'file':
