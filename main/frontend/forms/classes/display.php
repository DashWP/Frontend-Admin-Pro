	...
	class Display_Form {
		...
		public function validate_form( $form ) {
			...
			$form = feadmin_parse_args(
				...
				array(
					...
					'field_el'              => 'div',
					'uploader'              => ! empty ( $form['wp_uploader'] ) ? 'wp' : 'basic',
					'honeypot'              => true,
			...
			if ( 'wp' == $form['uploader'] && ! feadmin_edit_mode() ) {
		...
		public function form_set_data( $form = array() ) {
			...
			$data = wp_parse_args(
				...
				array(
					...
					'status'     => '',
					'message'    => '',
					'required_message' => $form['required_message'] ?? '',
		...
		function get_field_value( $field, $form = array() ) {
			...
			if( ! $id ){
				if ( $type == 'options' ){
					...
					$id = $form[ "{$type}_id" ] ?? get_the_ID();
				}
				
			}

			if ( 'post' == $type || 'product' == $type || ! is_numeric( $id ) ) {
				$object_id = $id;
			} else {
				$object_id = $type . '_' . $id;
			}
		...
		function render_submission_item( $submission ) {
			...
			if ( empty( $submission['submission_title'] ) ) {
				$submission['submission_title'] = sprintf( __( 'Submission #%d', 'acf-frontend-form-element' ), $submission['id'] );
			}
			...
			?>
			<a href="<?php echo esc_attr( $submission_link ); ?>"><h4 class="item-title"><?php echo esc_html( $submission['submission_title'] ); ?></h4></a>
